  ...
  return <div className={`auth-page ${register ? 'auth-register' : 'auth-login'}`}>
    ...
    <main className="auth-main">
      ...
      <section className="auth-panel" aria-label={register ? 'Registration form' : 'Login form'}>
        ...
        {!register && <Link className="auth-create-link" to="/register">Create Account</Link>}
        ...
        {register ? <p className="auth-switch">Already have an account? <Link to="/login">Log In</Link></p> : <><Link className="auth-guest" to="/frontend">Continue as Guest <ArrowRight size={17} /></Link><p className="auth-legal">By continuing, you agree to the <button className="auth-text-link" onClick={() => info('Terms & Privacy Policy', 'Our full terms and privacy policy will be available before account registration opens.')}>Terms & Privacy Policy</button></p></>}
      </section>
    </main>
