...
export default function Landing() {
  ...
  useEffect(() => { const query = new URLSearchParams(search); setFilter(query.has('favorites') ? 'favorites' : 'all'); setExpanded([]); if (query.has('favorites') || query.has('catalog')) { const frame=requestAnimationFrame(()=>catalog.current?.scrollIntoView({block:'start'})); return ()=>cancelAnimationFrame(frame) } }, [search, key])
  ...
  function choose(next: Filter) { setFilter(next); setExpanded([]) }
