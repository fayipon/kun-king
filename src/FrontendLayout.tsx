...
import { ArrowLeft, Crown, Gift, Heart, House, UsersRound, UserRound, Wallet, X } from 'lucide-react'
...
function BottomNav() {
  ...
  const active = location.pathname === '/promo' ? 'promo' : new URLSearchParams(location.search).has('favorites') ? 'my' : 'home'
  ...
  return <><nav className="kk-bottom-nav" aria-label="Lobby navigation">{[{ id:'home',label:'Home',icon:House },{ id:'promo',label:'Promo',icon:Gift },{ id:'wallet',label:'Wallet',icon:Wallet },{ id:'affiliate',label:'Affiliate',icon:UsersRound },{ id:'my',label:'My',icon:UserRound }].map(({id,label,icon:Icon}) => <button key={id} className={`${(panel ? panel.toLowerCase() : active) === id ? 'active ' : ''}${id === 'wallet' ? 'kk-center-nav' : ''}`} aria-current={!panel && active === id ? 'page' : undefined} aria-expanded={id === 'home' || id === 'promo' ? undefined : panel === label} onClick={e => { if (id === 'home' || id === 'promo') { close(); navigate(id === 'home' ? '/frontend' : '/promo'); window.scrollTo({ top:0, behavior:'instant' }) } else { opener.current=e.currentTarget; setPanel(label) } }}><span><Icon size={id === 'wallet' ? 27 : 20} /></span>{label}</button>)}</nav><dialog className="kk-dialog kk-nav-dialog" ref={dialog} aria-labelledby="nav-panel-title" onCancel={e=>{e.preventDefault();close()}} onClick={e=>{if(e.target===e.currentTarget)close()}}>{panel && <div className="kk-dialog-content"><button className="kk-dialog-close" aria-label="Close menu" onClick={close}><X size={21}/></button><h2 id="nav-panel-title">{panel}</h2>{panel === 'My' ? <div className="kk-account-actions"><Link to="/login" className="kk-primary" onClick={close}>Log In</Link><Link to="/register" className="kk-primary" onClick={close}>Register</Link><button className="kk-primary" onClick={()=>{close();navigate('/frontend?favorites=1')}}>My Favorites<Heart size={17}/></button></div> : <><p>{panel === 'Wallet' ? 'Your wallet will be available when account services launch.' : 'Our affiliate program is coming soon.'}</p><button className="kk-primary" onClick={close}>Got it</button></>}</div>}</dialog></>
}
