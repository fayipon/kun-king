...
function BottomNav() {
  ...
  const active = location.pathname === '/wallet' ? 'wallet' : location.pathname === '/my' ? 'my' : location.pathname === '/promo' ? 'promo' : new URLSearchParams(location.search).has('favorites') ? 'my' : 'home'
  ...
  return <><nav className="kk-bottom-nav" aria-label="Lobby navigation">{[{ id:'home',label:'Home',icon:House },{ id:'promo',label:'Promo',icon:Gift },{ id:'wallet',label:'Wallet',icon:Wallet },{ id:'affiliate',label:'Affiliate',icon:UsersRound },{ id:'my',label:'My',icon:UserRound }].map(({id,label,icon:Icon}) => <button key={id} className={`${(panel ? panel.toLowerCase() : active) === id ? 'active ' : ''}${id === 'wallet' ? 'kk-center-nav' : ''}`} aria-current={!panel && active === id ? 'page' : undefined} aria-expanded={id === 'home' || id === 'promo' || id === 'my' || id === 'wallet' ? undefined : panel === label} onClick={e => { if (id === 'home' || id === 'promo' || id === 'my' || id === 'wallet') { close(); navigate(id === 'home' ? '/frontend' : id === 'my' ? '/my' : id === 'wallet' ? '/wallet' : '/promo'); window.scrollTo({ top:0, behavior:'instant' }) } else { opener.current=e.currentTarget; setPanel(label) } }}><span><Icon size={id === 'wallet' ? 27 : 20} /></span>{label}</button>)}</nav><dialog className="kk-dialog kk-nav-dialog" ref={dialog} aria-labelledby="nav-panel-title" onCancel={e=>{e.preventDefault();close()}} onClick={e=>{if(e.target===e.currentTarget)close()}}>{panel && <div className="kk-dialog-content"><button className="kk-dialog-close" aria-label="Close menu" onClick={close}><X size={21}/></button><h2 id="nav-panel-title">{panel}</h2><p>Our affiliate program is coming soon.</p><button className="kk-primary" onClick={close}>Got it</button></div>}</dialog></>
}
