...
import { Bell, Settings, ArrowLeft, Crown, Gift, House, UsersRound, UserRound, Wallet, X } from 'lucide-react'
import './landing/landing.css'
import './frontend.css'

function Header() {
  const { pathname } = useLocation()
  return <header className="kk-header"><Link to={pathname === '/frontend' ? '/' : '/frontend'} className="kk-back" aria-label={pathname === '/frontend' ? 'Back to portal' : 'Back to game lobby'}><ArrowLeft size={18} /></Link><Link to="/frontend" className="kk-logo"><span><Crown size={23} /></span><div>KUN<span>KING</span><small>YOUR PLAYGROUND</small></div></Link>{pathname === '/my' ? <div className="kk-header-tools my-header-actions"><Link to="/my?panel=notifications" aria-label="Notifications"><Bell size={23}/><i/></Link><Link to="/my?panel=preferences" aria-label="Settings"><Settings size={23}/></Link></div> : <div className="kk-header-tools kk-auth-links"><Link to="/login">Log In</Link><Link to="/register">Register</Link></div>}</header>
}
function BottomNav() {
