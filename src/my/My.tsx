...
export default function My() {
 ...
 return <main className="kk-main my-main">
   ...
   <section aria-labelledby="my-account"><div className="my-section-heading"><h2 id="my-account"><UserRound size={21}/>Account</h2></div><div className="my-menu">{entries.map(({id,label,icon:Icon,status})=><button key={id} aria-label={label} onClick={()=>id==='wallet'?navigate('/wallet'):open(id)}><Icon size={21}/><span>{label}</span>{status&&<small className={id==='verification'?'verified':''}>{status}</small>}<ChevronRight size={16}/></button>)}</div></section>
